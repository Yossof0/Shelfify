...
import { InsertUser, users, products, purchases, purchaseItems, Product, InsertProduct, Purchase, InsertPurchase, PurchaseItem, InsertPurchaseItem } from "../drizzle/schema";
...
export async function createProduct(product: Omit<InsertProduct, 'createdAt' | 'updatedAt'>) {
  const db = await getDb();
...
export async function updateProduct(id: number, updates: Partial<Omit<Product, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function deleteAllProductsByUserId(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
// Purchase items queries
export async function createPurchaseItem(item: InsertPurchaseItem) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return db.insert(purchaseItems).values(item);
}
